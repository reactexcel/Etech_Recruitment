...
Meteor.methods({
    "fetchAllAction": function() {
        ...
        _.map(actions, (data, key) => {
            ...
            let dependentAction = DynamicActions.find({
                _id: data.dependentAction
            }).fetch();
            ...
            newActionList.push({
                ...
                "commentToggle": data.commentToggle,
                "tag_name": tag[0].name,
        ...
        if (id != '') {
            let _id = DynamicActions.update({
                ...
                $set: {
                    ...
                    commentToggle: action.commentToggle
                }
            })
            return _id;
        } else {
    ...
    "candidateActionTaken": function(A_id, email_ids, key, value) {
        let action = DynamicActions.find({
            _id: A_id
        }).fetch()
        let template = EmailTemplates.find({
            _id: action[0].template_id
        }).fetch()
        let tag = Tags.find({
            _id: action[0].tag_id
        }).fetch()
        ...
        _.map(email_ids, (emailId) => {
            ...
            try {
                ...
                //---
                let history = CandidateHistory.find({
                ...
                let updatedHistory = CandidateHistory.find({
                    email_id: email[0]._id
                }).fetch()
                prograsStatus.push({
                    emailId: email[0]._id,
                    progress: (candidatePoints / totalPoints * 100),
                    candidateAction: updatedHistory[0].dynamicActions
                })
