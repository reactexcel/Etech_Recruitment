...
Meteor.methods({
  ...
  "scheduleMultipleCandidate": function (idList, tagId, userId){
    let username=Meteor.users.findOne({"_id": userId})
    var mail;
    var email_id;
    var currentDate = new Date();
    var newIdList = [];
     _.map(idList,(id)=>{
      email_id = CandidateHistory.find({"email_id":id}).fetch()
       ...
       if(_.includes(mail[0].tags,tagId)==false){
         Logs.insert({
                action_type:"Candidate is moved to schedule tag",
                user_id:userId,
                details:"Action apply on Candidate : "+mail[0].from,
                username:username.username,
                created_on:new Date()
             ...
             if(email_id.length===0){
                 CandidateHistory.insert({
                   ...
                   historyDetails:[{
                    "ignored":true,
                    "date":currentDate,
                    "detail":"Candidate is Moved to schedule tag",
                    "actionPerformedBy":username.username
                  }]
                 });
             }else{
                CandidateHistory.update(
                  { email_id: id },
                  { $addToSet: {'historyDetails':{"scheduled":true,"date":currentDate,"detail":"Candidate is Moved to schedule tag","actionPerformedBy":username.username}}}
                );
             }
       }
